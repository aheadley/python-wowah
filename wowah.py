...
class Auction(AuctionDataModel):
    ...
    def json_obj2model_dict(cls, obj, ts):
        model_dict = {
            'started_at': ts,
            'item_extra': json.dumps({k: v for k,v in obj.iteritems() \
                    if k not in cls.ITEM_EXTRA_IGNORE_KEYS}) \
                or None,
        }
...
class DataManager(object):
    def run(self, data_src, batch_size=50, day_buffer=7):
        # aids = Auction IDs
        for ts, data in data_src:
            active_aids = set(a.auc_id for a in Auction.select().where(
                (Auction.is_active == True) &
                (Auction.started_at >= (ts - datetime.timedelta(days=day_buffer))) &
...
if __name__ == '__main__':
    ...
    DB_PROXY.initialize(db_connect(db_addr))
    DB_PROXY.create_tables(MODELS, safe=True)

    ds = DataSource(data_path)
    dm = DataManager()
    dm.run(ds, batch_size=opts.batch_size, day_buffer=opts.day_buffer)
